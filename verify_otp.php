...
<?php 
	if (isset($_SESSION['emp'])) { //check if the user who logs in is a bank personnel
		...
		if (!isset($_REQUEST['submitOTP'])) {
			...
			#####
			$sms = new Sms("$senderAddress", "$accessToken");// The codes below, where i put //, are the codes for globe send sms.
			$sms->setReceiverAddress("$mobile");// This is the most needed part. The senderAddress is the mobile number
			$sms->setMessage("$msg");//I mean, its a response. see documentation.
